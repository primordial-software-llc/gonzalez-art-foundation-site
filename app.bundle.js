...
class Artists {
  ...
  createFilterButtons(selectedLetter, search) {
    ...
    ranges.forEach((range, idx) => {
      ...
      for (let i = range.start; i <= range.end; i++) {
        ...
        const link = $(`<a href="${window.location.pathname}?letter=${letter}" class="btn btn-outline-primary${isActive ? ' active' : ''}" data-letter="${letter}"${isActive ? ' aria-current="page"' : ''}>${letter.toUpperCase()}</a>`);
  ...
  searchArtists(artists, query) {
    ...
    let artistList = $('<ul class="artist-list"></ul>'); // If query is less than 3 characters, show all artists
  ...
  init() {
    ...
    $('#artist-search-input').val(search); // Create filter buttons
